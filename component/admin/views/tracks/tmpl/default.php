...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_wsmusic&task=tracks.saveOrderAjax&tmpl=component';
			...
			<table class="table table-striped" id="trackList">
				<thead>
					<tr>
						...
						<th width="10%" class="nowrap hidden-phone">
							<?php echo JHtml::_('searchtools.sort', 'JGRID_HEADING_ARTIST', 'artist', $listDirn, $listOrder); ?>
						</th>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid; ?>">
						...
						<td class="center">
							<div class="btn-group">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'tracks.', $canChange, 'cb', $item->publish_up); ?>

								<?php
								// Create dropdown items
								$action = $archived ? 'unarchive' : 'archive';
								JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'tracks');
						...
						<td class="has-context">
							<div class="pull-left">
								<?php if ($item->artist == '*'):?>
									<?php $artist = JText::alt('JALL', 'artist'); ?>
								<?php else:?>
									<?php $artist = $item->artist_name ? $this->escape($item->artist_name) : JText::_('JUNDEFINED'); ?>
								<?php endif;?>
								<?php if ($canEdit || $canEditOwn) : ?>
									<a href="<?php echo JRoute::_('index.php?option=com_wsmusic&task=track.edit&id=' . $item->id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
						...
						<td class="small hidden-phone">
							<?php if ($item->artist == '*'):?>
								<?php echo JText::alt('JALL', 'artist'); ?>
							<?php else:?>
								<a href="<?php echo JRoute::_('index.php?option=com_wsmusic&task=artist.edit&id=' . $item->artist_id); ?>" title="<?php echo JText::_('JACTION_EDIT'); ?>">
								<?php echo $this->escape($item->artist_name); ?></a>
							<?php endif;?>
						</td>
